...
class CRM_Tournament_Page_TournamentInfo extends CRM_Event_Page_EventInfo {
	public function run() {
		//get the event id.
		...
		$breadCrumbPath = CRM_Utils_System::url('civicrm/tournament/info',
				"id={$this->_id}&reset=1"
				);
		...
		if ($participantListingID) {
			$participantListingURL = CRM_Utils_System::url('civicrm/tournament/participant',
					"reset=1&id={$this->_id}",
					TRUE, NULL, TRUE, TRUE
					);
		...
		if (!empty($values['event']['is_online_registration'])) {
			if (CRM_Event_BAO_Event::validRegistrationRequest($values['event'], $this->_id)) {
				...
				$url = CRM_Utils_System::url('civicrm/tournament/register',
			...
			if (CRM_Event_BAO_Event::checkRegistration($params)) {
				if ($noFullMsg == 'false') {
					if ($values['event']['allow_same_participant_emails']) {
						...
						$registerUrl = CRM_Utils_System::url('civicrm/tournament/register',
